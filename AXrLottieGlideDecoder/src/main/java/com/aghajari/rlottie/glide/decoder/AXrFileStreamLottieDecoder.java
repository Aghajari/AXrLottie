 ...
 * {@link AXrLottieDrawable} from {@link InputStream} data
 */
public class AXrFileStreamLottieDecoder implements ResourceDecoder<FileInputStream, AXrLottieDrawable> {

    @Override
    public boolean handles(@NonNull FileInputStream inputStream, @NonNull Options options) throws IOException {
        return AXrLottieGlideLibraryModule.handles(options);
    }

    @Nullable
    @Override
    public Resource<AXrLottieDrawable> decode(@NonNull FileInputStream inputStream, int width, int height, @NonNull Options options) throws IOException {
