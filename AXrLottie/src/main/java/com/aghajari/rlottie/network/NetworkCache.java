...
public class NetworkCache {
     ...
     * <p>
     * Once the animation is successfully parsed, {@link #renameTempFile(Context, String, FileExtension)} must be
     * called to move the file from a temporary location to its permanent cache location so it can
     * be used in the future.
     */
    @Nullable
    @WorkerThread
    public static Pair<FileExtension, File> fetch(Context context, String cacheName,FileExtension[] extensions) {
        final File cachedFile = getCachedFile(context,cacheName,extensions);
        if (cachedFile == null) {
            return null;
        }
     ...
     * to an composition, {@link #renameTempFile(Context, String, FileExtension)} should be called to move the file
     * to its final location for future cache hits.
     */
    public static File writeTempCacheFile(Context context,String cacheName, InputStream stream, FileExtension extension) throws IOException {
        String fileName = filenameForUrl(cacheName, extension, true);
    ...
    /**
     * If the file created by {@link #writeTempCacheFile(Context, String, InputStream, FileExtension)} was successfully parsed,
     * this should be called to remove the temporary part of its name which will allow it to be a cache hit in the future.
     */
    public static File renameTempFile(Context context, String cacheName, FileExtension extension) {
        String fileName = filenameForUrl(cacheName,extension, true);
        File file = new File(context.getCacheDir(), fileName);
    ...
    public static File loadTempFile(Context context,String cacheName){
        return NetworkCache.renameTempFile(context, cacheName, FileExtension.JSON());
    }
     ...
     */
    @Nullable
    public static File getCachedFile(Context context, String cacheName, FileExtension[] extensions) {
        for (FileExtension extension : extensions){
            File file = new File(context.getCacheDir(), filenameForUrl(cacheName, extension, false));
            if (file.exists()) {
                return file;
            }
        }
    ...
    public static String filenameForUrl(String cacheName, FileExtension extension, boolean isTemp) {
